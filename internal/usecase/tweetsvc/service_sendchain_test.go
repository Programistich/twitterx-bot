...
func (s *chainSender) SendTweet(_ context.Context, _ int64, _ int64, _ *twitterxapi.Tweet, _ *tweet.SendResponseOpts) error {
	return nil
}

func (s *chainSender) SendChainResponse(chatID int64, items []chain.ChainItem, replyToMsgID int64, opts *tweet.SendChainResponseOpts) error {
	s.calls++
	s.got.chatID = chatID
...
func TestServiceSendChainSuccess(t *testing.T) {
	...
	sender := &chainSender{}
	svc := New(fetcher, sender)

	err := svc.SendChain(context.Background(), 11, 22, "alice", "200", "@req")
	if err != nil {
...
func TestServiceSendChainParentMissing(t *testing.T) {
	root := &twitterxapi.Tweet{
		ID:               "200",
		Text:             "root",
		URL:              "https://x.com/alice/status/200",
...
func TestServiceSendChainFetchError(t *testing.T) {
	fetcher := &chainFetcher{
		errs: map[string]error{
...
func TestServiceSendChainSenderError(t *testing.T) {
	fetcher := &chainFetcher{
		tweets: map[string]*twitterxapi.Tweet{
			"alice:200": {ID: "200", Text: "root", URL: "https://x.com/alice/status/200"},
		},
	}
	sender := &chainSender{err: errors.New("send fail")}
	svc := New(fetcher, sender)
