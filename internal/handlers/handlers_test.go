...
func TestTwitterURLRegex(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			matches := twitterURLRegex.FindStringSubmatch(tt.input)
			...
			if tt.wantMatch {
				...
				if len(matches) < 3 {
					t.Errorf("expected 3 groups, got %d", len(matches))
					return
				}
				if matches[1] != tt.wantUsername {
					t.Errorf("username: expected %q, got %q", tt.wantUsername, matches[1])
				}
				if matches[2] != tt.wantTweetID {
					t.Errorf("tweetID: expected %q, got %q", tt.wantTweetID, matches[2])
				}
			} else {
				if matches != nil {
